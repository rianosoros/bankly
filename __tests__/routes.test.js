...
//TESTS BUG #2
describe("PATCH /users/[username]", function() {
  test("should deny access if unauthorized user tries to update another user's information", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({
        _token: tokens.u2, //User u2 is not the owner or an admin
...
describe("DELETE /users/[username]", function() {
  ...
  test("should delete the user from the database", async function() {
    ...
    const deletedUser = await User.get("u1");
...
describe("authUser middleware", function() {
  test("should return 401 if token is missing", async function() {
  ...
  test("should return 401 if token is invalid", async function() {
    const response = await request(app)
      .get("/example-route")
      .query({ _token: "invalid_token" });
    expect(response.statusCode).toBe(401);
    expect(response.body).toEqual({
      status: 401,
  ...
  test("should set curr_username and curr_admin if token is valid", async function() {
    //'valid_token' is a valid token for a user
    const response = await request(app)
      .get("/example-route")
      .query({ _token: "valid_token" });
    expect(response.statusCode).toBe(200);

    expect(response.req.curr_username).toBe("username_from_token");
    expect(response.req.curr_admin).toBe(true); //user is an admin
  });
});
